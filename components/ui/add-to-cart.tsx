...
interface AddToCartProps {
  shoe: Shoe | null;
  onAdd: () => void;
  startCartAni?: boolean;
}

const AddToCart = ({ shoe, onAdd, startCartAni }: AddToCartProps) => {
  ...
  return (
    <>
      <motion.div
        ...
        className="relative bg-slate-500 w-fit cursor-pointer"
        whileTap={{ scale: 0.8 }}
      >
        <Button
          className="bg-[#164055] text-lg text-white flex items-center z-20 relative py-8 px-7 gap-2 cursor-pointer"
          onClick={onAdd}
        >
          {/* SVG Cart */}
